/**
 * Todozi System Prompts for AI Models (JavaScript)
 * This file contains system prompts that teach AI models how to use Todozi tags and JSON tool calls
 *
 * Enhancements:
 * - Template literals standardized
 * - Optional chaining applied
 * - Runtime validation for enums
 * - TagValidationError with field attribution
 * - JSDoc types (lightweight)
 * - Version constant
 * - Clean UMD export for broad compatibility
 */
...
(function (root, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    // Browser globals or ES modules
    if (root) {
      root.TodoziPrompts = factory();
    } else {
      // ES module context - use globalThis
      if (typeof globalThis !== 'undefined') {
        globalThis.TodoziPrompts = factory();
      } else if (typeof window !== 'undefined') {
        window.TodoziPrompts = factory();
      } else if (typeof global !== 'undefined') {
        global.TodoziPrompts = factory();
      }
    }
  }
}(typeof self !== "undefined" ? self : (typeof globalThis !== "undefined" ? globalThis : (typeof this !== "undefined" ? this : {})), function () {
  "use strict";

  const VERSION = "1.0.0";

  /**
   * JSDoc typedefs (lightweight)
  ...
  /**
   * Custom error for tag validation issues
   */
  ...
  // Constants: Enums (frozen)
  const PRIORITIES = Object.freeze(["low", "medium", "high", "critical", "urgent"]);
  const STATUSES = Object.freeze(["todo", "in_progress", "blocked", "review", "done"]);
  const ASSIGNEES = Object.freeze(["ai", "human", "collaborative"]);
  const MEMORY_TYPES = Object.freeze(["standard", "secret", "human", "short", "long", "emotions"]);
  const MEMORY_TERMS = Object.freeze(["short", "long"]);
  const MEMORY_IMPORTANCE = Object.freeze(["low", "medium", "high", "critical"]);
  const IDEA_SHARES = Object.freeze(["private", "team", "public"]);
  const IDEA_IMPORTANCE = Object.freeze(["low", "medium", "high", "breakthrough"]);
  const ERROR_SEVERITY = Object.freeze(["low", "medium", "high", "critical"]);
  const FEEL_INTENSITY = Object.freeze(Array.from({ length: 10 }, (_, i) => String(i + 1))); // "1".."10"
  ...
  const TOOL_NAMES = Object.freeze([
    "create_task", "create_memory", "create_idea",
    "search_tasks", "update_task", "simple_todozi"
  ]);

  /**
   * Runtime enum validator
   * @param {string} value
  ...
  function assertEnum(value, allowed, field) {
    if (typeof value !== "string") {
      throw new TagValidationError(`Expected string for ${field}`, field);
    }
    if (!allowed.includes(value)) {
      throw new TagValidationError(`Invalid value "${value}" for ${field}. Allowed: ${allowed.join(", ")}`, field);
    }
    return true;
  }
  ...
  /**
   * Normalize semicolon-separated tag parameters (trim and collapse spaces)
   * @param {string} raw
   * @returns {string}
   */
  function normalizeTagParams(raw) {
    return String(raw)
      .split(";")
      .map(s => s.trim())
      .join("; ");
  ...
  // System Prompts (template literals standardized)
  const SYSTEM_PROMPT_TAG_BASED = `You are an AI assistant integrated with Todozi, a comprehensive task and knowledge management system.
...
`;
  ...
  const SYSTEM_PROMPT_JSON_TOOLS = `You are an AI assistant integrated with Todozi, a comprehensive task and knowledge management system.
...
`;
  ...
  const QUICK_TAG_REFERENCE = `## QUICK TAG REFERENCE
...
`;
  ...
  // Enhanced prompts
  const SYSTEM_PROMPT_TAG_BASED_ENHANCED = `You are an AI assistant deeply integrated with Todozi, a comprehensive task and knowledge management system that uses structured tags to organize thoughts, tasks, and knowledge.
...
`;
  ...
  const SYSTEM_PROMPT_JSON_ENHANCED = `You are an AI assistant integrated with Todozi through JSON tool calling. This is the FALLBACK interface when tags aren't appropriate or aren't working.
...
`;
  ...
  // Direct prompts
  const SYSTEM_PROMPT_TAGS_DIRECT = `You use Todozi tags to manage tasks and knowledge. Tags are primary. Be direct and efficient.
...
`;
  ...
  const SYSTEM_PROMPT_JSON_DIRECT = `JSON tools for Todozi. Use when tags fail or for complex operations.
...
`;
  ...
  const SYSTEM_PROMPT_ULTRA_DIRECT = `Todozi system. Tags first, JSON fallback.
...
`;
  ...
  const SYSTEM_PROMPT_MINIMAL = `Todozi: <todozi>action;time;priority;project;status</todozi> for tasks.
...
`;
  ...
  const QUICK_REFERENCE = `TODOZI QUICK REFERENCE
...
`;
  ...
  // Tag validation schemas with defaults
  const TAG_VALIDATION_SCHEMAS = Object.freeze({
    "todozi": Object.freeze({
      "required": Object.freeze(["action", "time", "priority", "project", "status"]),
      "optional": Object.freeze(["assignee", "tags", "dependencies", "context_notes", "progress"]),
    ...
    "memory": Object.freeze({
      "required": Object.freeze(["type", "moment", "meaning", "reason", "importance", "term"]),
      "optional": Object.freeze(["tags"]),
      "defaults": Object.freeze({
        "type": "standard",
        "importance": "medium",
        "term": "long"
    ...
    "idea": Object.freeze({
      "required": Object.freeze(["idea", "share", "importance"]),
      "optional": Object.freeze(["tags", "context"]),
      "defaults": Object.freeze({
        "share": "team",
        "importance": "medium"
      })
  ...
  /**
   * Validate parameters for a given tag schema
   * @param {string} tagName - One of "todozi", "memory", "idea"
   * @param {Object} params - Raw parameters object
   * @returns {ValidationResult}
   */
  function validateTagParameters(tagName, params) {
    const errors = [];
    const warnings = [];
    const normalized = {};
    const schema = TAG_VALIDATION_SCHEMAS[tagName];

    if (!schema) {
      throw new TagValidationError(`Unknown tag "${tagName}"`, "tagName");
    }
    ...
    // Normalize and validate known enums where applicable
    if (tagName === "todozi") {
      // priority
      if (params?.priority !== undefined) {
        try {
          assertEnum(sanitize(params.priority), PRIORITIES, "priority");
        } catch (e) {
      ...
      if (params?.status !== undefined) {
        try {
          assertEnum(sanitize(params.status), STATUSES, "status");
        } catch (e) {
          errors.push(e.message);
        }
      }
      ...
      if (params?.assignee !== undefined) {
        try {
          assertEnum(sanitize(params.assignee), ASSIGNEES, "assignee");
        } catch (e) {
          errors.push(e.message);
        }
      }
      ...
      if (params?.type !== undefined) {
        try {
          assertEnum(sanitize(params.type), MEMORY_TYPES, "type");
        } catch (e) {
          errors.push(e.message);
        }
      }
      if (params?.importance !== undefined) {
        try {
          assertEnum(sanitize(params.importance), MEMORY_IMPORTANCE, "importance");
        } catch (e) {
          errors.push(e.message);
        }
      }
      if (params?.term !== undefined) {
        try {
          assertEnum(sanitize(params.term), MEMORY_TERMS, "term");
        } catch (e) {
          errors.push(e.message);
        }
      }
    } else if (tagName === "idea") {
      if (params?.share !== undefined) {
        try {
          assertEnum(sanitize(params.share), IDEA_SHARES, "share");
        } catch (e) {
          errors.push(e.message);
        }
      }
      if (params?.importance !== undefined) {
        try {
          assertEnum(sanitize(params.importance), IDEA_IMPORTANCE, "importance");
        } catch (e) {
          errors.push(e.message);
        }
      }
   ...
   * @param {boolean} useTags - Whether to use tag-based system
   * @returns {string} The system prompt string
   */
  function getSystemPrompt(useTags = true) {
    return useTags ? SYSTEM_PROMPT_TAG_BASED : SYSTEM_PROMPT_JSON_TOOLS;
  }

  /**
   * Get examples of Todozi tags for learning
   * @returns {string} Examples of Todozi tags
   */
...
`;
   ...
   * @returns {Array} Array of JSON tool examples
   */
  function getJsonToolExamples() {
    return [
      {
        "type": "function",
        "function": {
   ...
   * @param {boolean} useTags - Whether to prioritize tag-based system
   * @param {Object} [userContext] - Optional dict with user preferences/context
   * @returns {string} The enhanced system prompt string
   */
  function getEnhancedSystemPrompt(useTags = true, userContext = null) {
    let basePrompt = useTags ? SYSTEM_PROMPT_TAG_BASED_ENHANCED : SYSTEM_PROMPT_JSON_ENHANCED;

    // Optional chaining applied here (feedback suggestion)
    if (userContext?.style !== undefined || userContext?.project !== undefined || userContext?.expertise !== undefined || userContext?.teamSize !== undefined) {
      const contextAddition = `
## 📝 USER CONTEXT
- Preferred style: ${userContext?.style || 'balanced'}
- Primary project: ${userContext?.project || 'general'}
- Expertise level: ${userContext?.expertise || 'intermediate'}
- Team size: ${userContext?.teamSize || 'solo'}
`;
   ...
   * @param {string} modelType - Model type: "direct", "verbose", "minimal", "ultra", "structured"
   * @returns {string} The optimized system prompt
   */
  function getModelOptimizedPrompt(modelType = "direct") {
    const prompts = {
      "direct": SYSTEM_PROMPT_TAGS_DIRECT,
      "verbose": SYSTEM_PROMPT_TAG_BASED_ENHANCED,
...
`
    ...
    return prompts[modelType] || SYSTEM_PROMPT_TAGS_DIRECT;
  }
  ...
  return {
    ...
    // Model configurations for context window management
    MODEL_CONFIGS: {
      'default': {
        contextLimit: 4096,
        contextRatio: 0.8
...
// ES Module export support - extract the UMD result
// The UMD wrapper above sets module.exports or globalThis.TodoziPrompts
let TodoziPromptsExport = {};
try {
  if (typeof module !== 'undefined' && module.exports && Object.keys(module.exports).length > 0) {
    TodoziPromptsExport = module.exports;
  } else if (typeof globalThis !== 'undefined' && globalThis.TodoziPrompts) {
    TodoziPromptsExport = globalThis.TodoziPrompts;
  } else if (typeof self !== 'undefined' && self.TodoziPrompts) {
    TodoziPromptsExport = self.TodoziPrompts;
  }
} catch (e) {
  // Fallback if extraction fails - use empty object
  TodoziPromptsExport = {};
}

// ES module default export
export default TodoziPromptsExport;